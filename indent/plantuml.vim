...
" Author:
" =======
" Dean Ashton, dean.ashton@gmail.com
" Based on Awk script by :  Erik Janssen, ejanssen@itmatters.nl
...
function! GetPlantUMLIndent()
   ...
   if s:Starts_with_optword( prev_data )
      return ind + &sw
   endif

   if s:Starts_with_loopword( prev_data )
      return ind + &sw
   endif

   if s:Starts_with_endword( getline(v:lnum) )
      return ind - &sw
   endif
...
function! s:Starts_with_optword( line )
  if a:line =~ '^\s*opt\>'
     return 1
  endif
  return 0
...
function! s:Starts_with_loopword( line )
  if a:line =~ '^\s*loop\>'
     return 1
  endif
  return 0
